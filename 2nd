(() => {
  ...
  const CONFIG = {
    ...
    INITIAL_MESSAGES: ["This is a demo version of the chatbot you'll have on your website. Pretend you're a client in need of legal representation and see how it reacts!", "This is Legenix Law Firm's confidential chat. How may we help you today?"],
    WEBHOOK_URL: 'https://n8n.legenix.com/webhook/legenix',
  ...
  class RBChatWidget extends HTMLElement {
    constructor(){
      ...
      this.shadowRoot.innerHTML = `
        <style>
          ...
          .rb-welcome{
            ...
            background:#fff;color:#111;padding:12px 14px;border-radius:12px;box-shadow:0 0 30px rgba(0,0,0,.28);
            font-size:14px;max-width:min(35vw,320px);z-index:2147483645;line-height:1.3;
          }
          .rb-welcome.hide{display:none}
          .rb-welcome:after{content:"";position:absolute;bottom:-10px;right:20px;border:10px solid transparent;border-top-color:#fff;filter:drop-shadow(0 0 6px rgba(0,0,0,.12))}
          .rb-overlay{
        ...
        <div class="rb-root">
          <div class="rb-launcher" id="rb-launcher" aria-controls="rb-chat" aria-expanded="false" title="Chat">
            ...
            <span class="launcher-x" id="launcher-x" aria-hidden="true">✕</span>
          </div>
          ...
          <section class="rb-chat" id="rb-chat" role="dialog" aria-modal="true" aria-labelledby="rb-title">
            <header class="rb-head">
              ...
              <button class="rb-close" id="rb-close" aria-label="Close">✕</button>
            </header>
            ...
            <footer class="rb-foot">
              <div class="composer">
                <div class="input-row">
                  <input id="rb-input" placeholder="Type a new message…" autocomplete="off" />
      ...
      setSrc('logo-header', ASSETS.HEADER_LOGO_URL, '◇');
      setSrc('logo-launcher',ASSETS.LAUNCHER_LOGO_URL,'💬');
      setSrc('logo-send', ASSETS.SEND_BUTTON_LOGO_URL,'➤');
      ...
      const doSend = async () => {
        ...
        try{
          ...
          hideTyping(); addMsg(String(reply||'…'),'bot');
        }catch(e){
          hideTyping(); addMsg("Sorry—couldn’t reach the server. Please try again.",'bot');
          console.error(e);
        }finally{
      ...
      inputEl.addEventListener('blur', () => { if (!inputEl.value.trim()) inputEl.placeholder = 'Type a new message…'; });
